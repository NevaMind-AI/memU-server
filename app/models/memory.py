...
class MemoryBase(SQLModel):
    ...
    embedding: list[float] | None = Field(
        ...
        description="Memory embedding vector (1536 dimensions)"
    )
    links: list | None = Field(
        default=None,
        sa_column=Column(JSONB),
        description="Related links or references"
    )
...
class Memory(BaseModel, MemoryBase, table=True):
    ...
    # category_rel: "MemoryCategory | None" = Relationship(back_populates="memories")
