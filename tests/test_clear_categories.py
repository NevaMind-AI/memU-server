...
@pytest.fixture
def mock_service():
    """Mock MemoryService for endpoint tests."""
    with patch("app.main.service") as mock:
        ...
        mock.list_memory_categories = AsyncMock(
            return_value={
                "categories": [
                    {
                        "name": "work",
...
def test_clear_memory_success(mock_service, client):
...
def test_list_categories_success(mock_service, client):
...
def test_list_categories_service_error(mock_service, client):
    ...
    response = client.post("/categories", json={"user_id": "user123"})
    assert response.status_code == 500
    assert "Failed to list categories" in response.json()["detail"]
